...
async function main() {
  ...
  console.log(`Created ${videos.length} curated videos`);

  // Load DRRC curated videos from JSON
  interface DrrcVideo {
    youtubeId: string;
    title: string;
    channelName: string;
  ...
  const drrcPath = join(__dirname, "drrc-videos.json");
  const drrcVideos: DrrcVideo[] = JSON.parse(readFileSync(drrcPath, "utf-8"));
  const existingYoutubeIds = new Set(videoData.map((v) => v.youtubeId));

  let drrcCount = 0;
  for (const dv of drrcVideos) {
    if (existingYoutubeIds.has(dv.youtubeId)) continue;
    await prisma.video.create({
      data: {
        youtubeId: dv.youtubeId,
        title: dv.title,
        description: `${dv.drrcCategory} content from ${dv.channelName}. Curated by DRRC for children's media.`,
        channelName: dv.channelName,
        ageMin: dv.ageMin,
        ageMax: dv.ageMax,
        category: dv.category,
        tags: JSON.stringify(dv.tags),
        thumbnailUrl: `https://img.youtube.com/vi/${dv.youtubeId}/mqdefault.jpg`,
        parentRating: 0,
        reviewCount: 0,
      },
    });
  ...
  console.log(`Created ${drrcCount} DRRC videos (${videos.length + drrcCount} total)`);
